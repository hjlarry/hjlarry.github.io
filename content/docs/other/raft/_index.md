...
那么怎样达成线性一致性呢？必须对所有的操作能找到一个统一的顺序，操作和实际的时间能匹配上(循环类操作除外)，并且每个读取都能按顺序的从其前面的写入中看到该值。例如:  

![](./images/linearizability.drawio.svg)

`Wx1`表示客户端A写入x的值为1，`Rx1`表示客户端B读取x的值得到1。对于某条线段，例如`Wx1`，它的起点表示客户端发起请求，终点表示服务端返回的时间，但是本身的读取、写入这些操作在服务器上的执行时间是很短的，相对于整个请求可以认为是瞬间完成，那么就可以认为是在时间轴上的一个点，这个点既可以在线段的最前面，也可以在最后面。我们可以从图2中看到x值的变化。